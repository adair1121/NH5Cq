...
var AssetsMgr = (function () {
     ...
     * @param:{thisArg}  回调作用域
     */
    AssetsMgr.prototype.loadSingleMovieFile = function (filePath, defaultRes, isLoop) {
        if (isLoop === void 0) { isLoop = false; }
        var movieClip = new MovieClip(isLoop);
        var mcFactory = new egret.MovieClipDataFactory();
        var isLoadTexture = false, isLoadJson = false;
        var fileName = filePath.split("/").pop();
        RES.getResByUrl(filePath + AssetsDefine.JsonSuffix + "?v=" + Config.MOVIEVERSION, function (json) {
        ...
        RES.getResByUrl(filePath + AssetsDefine.PngSuffix + "?v=" + Config.MOVIEVERSION, function (texture) {
     ...
     */
    AssetsMgr.prototype.loadMovieGroup = function (filePath, condition, id, numArr, isLoop) {
        var _this = this;
        if (isLoop === void 0) { isLoop = false; }
        var movieClipDic = new Dictionary();
        numArr.forEach(function (num) {
            var str = Utility.format(condition, id, num);
            var clip = _this.loadSingleMovieFile(filePath + str, null, isLoop);
