...
class AssetsMgr {
	 ...
	 * @param:{thisArg}  回调作用域
	 */
	public loadSingleMovieFile(filePath:string, defaultRes?:string, isLoop:boolean = false):MovieClip
	{
		var movieClip = new MovieClip(isLoop);

		var mcFactory:egret.MovieClipDataFactory = new egret.MovieClipDataFactory();
		var isLoadTexture:boolean = false, isLoadJson:boolean = false;
		var fileName:string = filePath.split("/").pop();

		RES.getResByUrl(filePath + AssetsDefine.JsonSuffix+"?v="+Config.MOVIEVERSION, json =>
		{
			mcFactory.mcDataSet = json;
			isLoadJson = true;

			if(isLoadTexture)
			{
				movieClip.name = fileName;
				movieClip.initMovieClipData(mcFactory.generateMovieClipData(fileName));
			}

		}, this, RES.ResourceItem.TYPE_JSON);

        RES.getResByUrl(filePath + AssetsDefine.PngSuffix+"?v="+Config.MOVIEVERSION, texture =>
		{
			mcFactory.texture = texture;
			isLoadTexture = true;
		...
		if (!movieClip.movieClipData && !!defaultRes)
			movieClip.movieClipData = new egret.MovieClipDataFactory(null, RES.getRes("defaultRes")).generateMovieClipData();
	 ...
	 */
	public loadMovieGroup(filePath:string, condition:string, id:number, numArr:Array<number>, isLoop:boolean = false):Dictionary<number, MovieClip>
	{
		var movieClipDic = new Dictionary<number, MovieClip>();

		numArr.forEach(num =>
		{
			let str = Utility.format(condition, id, num);
			let clip = this.loadSingleMovieFile(filePath + str, null, isLoop);
			movieClipDic.add(num, clip);
		});
